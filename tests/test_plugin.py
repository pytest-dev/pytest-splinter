...
def test_download_file(httpserver, browser, pytestbdd_file_download_dir):
    ...
    assert open(os.path.join(pytestbdd_file_download_dir, 'some.txt')).read() == 'Some text file'
...
@pytest.mark.parametrize('pytestbdd_webdriver', ['firefox', 'phantomjs'])
def test_get_text(httpserver, browser, pytestbdd_webdriver):
